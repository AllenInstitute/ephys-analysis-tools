...
def main():
	...
	master_jem_lims_df["test-jem_lims_specimen_id_congruency"] = np.where((master_jem_lims_df["jem-id_cell_specimen"] == master_jem_lims_df["lims-id_cell_specimen"])&(master_jem_lims_df["jem-id_patched_cell_container"] == master_jem_lims_df["lims-id_patched_cell_container"]), "Matching JEM & LIMS Information",
																 np.where((master_jem_lims_df["jem-id_cell_specimen"].isnull()), "Missing JEM Cell Specimen ID",
																 np.where((master_jem_lims_df["lims-id_cell_specimen"].isnull()), "Missing LIMS Cell Specimen ID",
																 np.where((master_jem_lims_df["jem-id_cell_specimen"] != master_jem_lims_df["lims-id_cell_specimen"]), "Mismatching JEM & LIMS Cell Specimen ID", "Unsure"))))
	# Create a new column (test-jem_lims) for master_jem_lims_df 
